...
namespace MediaPortal.Util
{
  ...
  public class Utils
  {
    ...
    public static void DownLoadAndCacheImage(string strURL, string strFile)
    {
      ...
      string mpImageCacheDirectory = Config.GetFolder(Config.Dir.Config) + @"\MPCache";

      try
      {
        if (!Directory.Exists(mpImageCacheDirectory))
        {
          Directory.CreateDirectory(mpImageCacheDirectory);
        }
      }
      catch (Exception ex)
      {
      ...
      string file = Path.Combine(mpImageCacheDirectory, url);
